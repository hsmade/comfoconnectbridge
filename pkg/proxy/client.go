...
func (c Client) Run(ctx context.Context, wg *sync.WaitGroup) error {
	helpers.StackLogger().Info("starting client")

	helpers.StackLogger().Debugf("starting new session with gateway %s", c.IP)
	session, err := comfoconnect.NewSession(ctx, wg, c.IP, 0, c.uuid)
	if err != nil {
		helpers.StackLogger().Errorf("failed to create a session with gateway %s: %v", c.IP, err)
	...
	for {
		...
		case message := <-c.toGateway:
			...
			helpers.StackLogger().WithField("span", span.Context().(jaeger.SpanContext).String()).Debugf("sending message to gateway: %v", message)
			err := c.session.Send(message)
			if err != nil {
				span.SetTag("err", err)
				helpers.StackLogger().Errorf("sending message to gateway failed: %v", err)
			}

			span.Finish()

		default:
			helpers.StackLogger().Debug("waiting for message from gateway")

			message, err := c.session.Receive()
			if err != nil {
				if errors.Cause(err) == io.EOF {
					helpers.StackLogger().Warn("gateway closed connection")
					clientConnected.Set(0)
				...
				helpers.StackLogger().Errorf("got error while receiving from gateway: %v", err)
				break // restart loop
			}
			clientMessagefromGateway.WithLabelValues(message.Operation.Type.String()).Inc()
			helpers.StackLogger().Debugf("received message from gateway: %v", message)
			...
			default:
				helpers.StackLogger().WithField("span", span.Context().(jaeger.SpanContext).String()).Debugf("sending message back to proxy: %v", message)
				c.fromGateway <- *message
				helpers.StackLogger().WithField("span", span.Context().(jaeger.SpanContext).String()).Debugf("sent message back to proxy: %v", message)
			}
			span.Finish()
		}
	}
